...
class EmailService {
  ...
  extractVerificationCode(emailContent, pattern = null) {
    ...
    const patterns = [
      // AWS 英文格式: "verification code is 123456" 或 "code: 123456"
      /(?:verification\s+code\s*(?:is)?|code\s*:)\s*(\d{6})\b/i,
      // AWS 格式: "Enter this code: 123456"
      /(?:enter\s+(?:this\s+)?code\s*:?)\s*(\d{6})\b/i,
      // 通用格式: 独立的 6 位数字（排除 000000 这种无效码）
      /\b([1-9]\d{5}|\d[1-9]\d{4}|\d{2}[1-9]\d{3}|\d{3}[1-9]\d{2}|\d{4}[1-9]\d|\d{5}[1-9])\b/
    ];
    ...
    for (const p of patterns) {
      const match = emailContent.match(p);
      if (match) {
        const code = match[1] || match[0];
        // 过滤掉明显无效的验证码
        if (code && code !== '000000' && !/^0+$/.test(code)) {
    ...
    // 最后尝试：找到所有 6 位数字，返回第一个非零的
    const allCodes = emailContent.match(/\b\d{6}\b/g) || [];
    for (const code of allCodes) {
      if (code !== '000000' && !/^0+$/.test(code)) {
